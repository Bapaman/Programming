...
def spin(window):
    layout = [
        ...
        [sg.Spin(values=[x for x in range(1, 96)], readonly=False, key='-SPIN-')],
        [sg.Button('OK', size=(12, 2), button_color=('white', 'purple'), key='-OK-')]
    ]
...
def gms_window():
    layout = [
        ...
        [sg.Spin(values=["Удалить по подстроке", "Удалить по началу названия", "Удалить по концу названия", "Удалить по расширению"], text_color='white', readonly=True, key='-DELETE LIST-')],
        [sg.Button('OK', size=(15, 3), button_color=('blue', 'white'), key='-OK-')]  # Изменения: размер кнопки (15, 3) и цвет (синий)
    ]
...
def choice_window(choice):
    layout = [
        ...
        [sg.Button('Удалить', size=(15, 3), button_color=('green', 'white'), key="-DELETE CHOICE-")]  # Изменения: размер кнопки (15, 3) и цвет (зеленый)
    ]
...
def office():
    sg.theme("DarkPurple")
    buttons1 = [[sg.Button('Действие', size=(12, 2), disabled=True, key="-ENTER-")],
                [sg.Button('Убрать', size=(12, 2), disabled=True, key='-CLEAR-')],
                [sg.Button('Убрать все', size=(12, 2), disabled=True, key='-CLEAR ALL-')]]
    buttons2 = [[sg.Button('Удаление файлов', size=(12, 4), disabled=True, key='-DELETE-')],
                [sg.Button('Удалить', size=(12, 2), disabled=True, key='-DELETE IN WINDOW-')]]

    file_list_column = [
        [sg.Text("File Change"), sg.In(size=(27, 1), enable_events=True, key="-FOLDER-"),
         sg.FolderBrowse()],
        [sg.Listbox(values=[], enable_events=True, size=(70, 20), key="-FILE LIST-")],
    ]
    image_viewer_column = [
        [sg.Text("Доступные действия:")],
        [sg.Text(size=(40, 5), key="-SHOW-")],
        [sg.Listbox(values=[], enable_events=True, size=(20, 10), key="-NEW LIST-")],
